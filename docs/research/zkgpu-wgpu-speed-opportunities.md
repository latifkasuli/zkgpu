...
> **Status (2026-04-30, pilot landed + benched on M4 Pro; NVIDIA pending).** The Immediate path is implemented for the Stockham radix-4 stage only, opt-in via `R4ParamMode::Immediate` (default stays `Storage`, mirroring item #6's pattern). M4 Pro / Metal A/B shows Immediate +2-5% **slower** at log_n ∈ {10, 14, 18}, **−11.6% faster** at log_n=20. The constant ~30-70 µs small-log_n regression looks encoder-side (per-stage `set_immediates` cost on the wgpu Metal backend); the log_n=20 win is real (CIs disjoint). Both vast.ai NVIDIA instances were unreachable when the bench ran, so the auto-detect decision is deferred until that data lands. R2 / local / Poseidon2 plans don't migrate yet. Full per-host A/B + reproducer: [`docs/research/r4-immediate-pilot-verdict.md`](r4-immediate-pilot-verdict.md).
