...
> **Status (2026-05-01, three-host A/B complete).** The Immediate path is implemented for the Stockham radix-4 stage only, opt-in via `R4ParamMode::Immediate` (default stays `Storage`, mirroring item #6's pattern). Three-host A/B (M4 Pro Metal + RTX 4090 Vulkan + RTX 5090 Vulkan) shows **no host with a uniform win**: 2/12 cells real win (M4 Pro log_n=20 −11.6%, RTX 5090 log_n=18 −1.7%), 4/12 real regression (M4 Pro log_n ∈ {10, 14, 18} +2-5%, RTX 4090 log_n=14 +6.3%), 6/12 within noise. The two consumer hot paths (Plonky3 `fri_commit` log_h=18, OpenVM mixed-height log_h_max=18-22) land in mixed-or-net-negative territory across hosts. **Production kernels keep the Storage path on every backend; R2 / local / Poseidon2 do not migrate.** R4 Immediate stays accessible via `WgpuNttPlan::new_with_r4_param_mode` for callers who measure on their own hardware. Full per-host A/B + reproducer: [`docs/research/r4-immediate-pilot-verdict.md`](r4-immediate-pilot-verdict.md).
...
> **Status (2026-05-01, narrow scope shipped).** The `R4ParamSource::Immediate` branch of `encode_ntt_stages` now builds 2 bind groups per encode (one per parity) instead of one per stage; cuts that branch's per-encode bind-group create cost from `O(num_r4_stages)` to `O(1)`. RTX 5090 Vulkan A/B at log_n=20 shows −2.8% Immediate-mode time (p=0.01); smaller log_n is below the noise floor. Storage mode and R2 / local / Poseidon2 / Four-Step plans are unchanged — those need per-stage bind groups because the per-stage `param_buffer` differs at slot 3, and the within-encode collapse only works in Immediate mode where slot 3 is gone. **The doc's broader scope** (pre-build at plan-build time, reuse across `execute()` calls) is deferred: it requires either an API change to own the user buffer in the plan, or a cross-execute cache. The infrastructure for the latter (`WgpuBuffer.id` field as a stable cache key) landed in `f23a7db` but the cache itself isn't wired pending evidence that consumer pipelines reuse the same buffer often enough for the hit rate to matter. Full verdict + reproducer: [`docs/research/item5-bind-group-reuse-verdict.md`](item5-bind-group-reuse-verdict.md).
